...
<section class="p-8 space-y-6">
    ...
    <form method="GET" action="{{ route('colaborador.aportaciones') }}" class="rounded-2xl border border-white/10 bg-black/30 p-4 grid gap-3 md:grid-cols-[1.2fr,1fr,1fr,auto] md:items-end">
        <div>
        ...
        <div>
            ...
            <input
                ...
                name="desde"
                value="{{ $desde }}"
                class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white focus:border-indigo-400 focus:ring-indigo-400"
            >
        </div>
        <div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('colaborador.aportaciones') }}" class="admin-btn admin-btn-ghost">
                Limpiar
            </a>
        </div>
    </form>
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Total aportado</p>
            <p class="text-2xl font-bold text-white">${{ number_format($totalAportado ?? 0, 2) }}</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Aportes</p>
            <p class="text-2xl font-bold text-white">{{ $numAportes ?? 0 }}</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Proyectos apoyados</p>
            <p class="text-2xl font-bold text-white">{{ $proyectosApoyados ?? 0 }}</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Última aportación</p>
            ...
            @foreach($aportaciones as $aporte)
                @php
                    ...
                    $estadoClase = match($estado) {
                        ...
                        default => 'bg-indigo-500/15 text-indigo-200 border-indigo-400/40',
                    };
                    ...
                    $borderAccent = match($estado) {
                        ...
                        default => 'border-l-4 border-indigo-400/60',
                    };
                @endphp
                <article class="rounded-2xl border border-white/10 bg-zinc-900/70 shadow-[0_18px_40px_rgba(0,0,0,0.35)] overflow-hidden {{ $borderAccent }}">
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 px-4 py-3 bg-white/5">
                        <div class="space-y-1">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ route('colaborador.aportaciones.recibo', $aporte) }}" class="inline-flex items-center gap-1 rounded-md border border-white/15 bg-white/10 px-3 py-1.5 text-[11px] font-semibold text-white hover:border-indigo-400/60">
                    ...
                    <details class="border-t border-white/5 px-4 py-3">
                        <summary class="flex items-center gap-2 text-xs font-semibold text-indigo-200 cursor-pointer">
