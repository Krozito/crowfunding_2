...
<section class="p-8 space-y-6">
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Proyectos</p>
            <p class="text-2xl font-bold text-white">{{ $totalProyectos }}</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Total recaudado</p>
            <p class="text-2xl font-bold text-white">${{ number_format($totalRecaudado, 0, ',', '.') }}</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Estado general</p>
            <p class="text-sm font-semibold text-emerald-200">Activo</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-inner space-y-1">
            <p class="text-[11px] uppercase tracking-[0.28em] text-indigo-200">Explorar</p>
    ...
    @if(isset($proyectosAportados) && $proyectosAportados->count())
        <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            @foreach($proyectosAportados as $proyecto)
                ...
                @endphp
                <article class="rounded-3xl border border-white/10 bg-zinc-950/80 shadow-[0_18px_45px_rgba(0,0,0,0.4)] overflow-hidden flex flex-col">
                    ...
                    <div class="p-5 space-y-3 flex-1 flex flex-col">
                        ...
                        <div class="flex justify-end">
                            <a href="{{ route('colaborador.proyectos.resumen', $proyecto) }}" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-bold uppercase tracking-wide text-white shadow-[0_10px_28px_rgba(79,70,229,0.35)] hover:bg-indigo-500 transition-colors">
                                Revisar
    ...
    @else
        <div class="rounded-3xl border border-white/10 bg-zinc-900/70 shadow-xl p-6 text-sm text-zinc-300">
            <p class="font-medium text-white mb-1">Aún no has realizado aportaciones.</p>
