...
<head>
    ...
    <title>Editar recompensa | CrowdUp Creador</title>
...
<body class="bg-zinc-950 text-zinc-100 font-sans min-h-screen">
    ...
    <header class="sticky top-0 z-30 border-b border-white/10 bg-zinc-950/80 backdrop-blur-xl">
        <div class="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-lg font-semibold text-white">Editar recompensa</h1>
            </div>
    ...
    <main class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <section class="rounded-3xl border border-white/10 bg-zinc-900/70 p-8 shadow-2xl ring-1 ring-indigo-500/15 space-y-4">
            <div>
                <h2 class="text-2xl font-bold text-white">Actualizar nivel de recompensa</h2>
                <p class="text-sm text-zinc-400">Ajusta proyecto, monto, descripcion y disponibilidad.</p>
            </div>
            ...
            <form method="POST" action="{{ route('creador.recompensas.update', $recompensa) }}" class="grid gap-4">
                @csrf
                @method('PATCH')
                <div>
                    ...
                    <select name="proyecto_id" required class="mt-1 w-full appearance-none rounded-xl border border-white/15 bg-zinc-900/80 px-4 py-2.5 text-sm text-white focus:border-indigo-400 focus:ring-indigo-400">
                        @forelse ($proyectos as $proyecto)
                            <option value="{{ $proyecto->id }}" @selected(old('proyecto_id', $recompensa->proyecto_id) == $proyecto->id)>{{ $proyecto->titulo }}</option>
                        @empty
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div>
                        ...
                        <input name="titulo" value="{{ old('titulo', $recompensa->titulo) }}" required class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white focus:border-indigo-400 focus:ring-indigo-400" placeholder="Early bird, Kit, VIP...">
                    </div>
                    <div>
                        <label class="text-xs text-zinc-400">Monto minimo (USD)</label>
                        <input name="monto_minimo_aportacion" type="number" min="0" step="1" value="{{ old('monto_minimo_aportacion', $recompensa->monto_minimo_aportacion) }}" required class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white focus:border-indigo-400 focus:ring-indigo-400" placeholder="25">
                    </div>
                </div>

                <div>
                    <label class="text-xs text-zinc-400">Descripcion</label>
                    <textarea name="descripcion" rows="3" class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white focus:border-indigo-400 focus:ring-indigo-400" placeholder="Que incluye este nivel y por que es atractivo">{{ old('descripcion', $recompensa->descripcion) }}</textarea>
                </div>

                <div class="grid gap-3 sm:grid-cols-[1fr,auto] sm:items-end">
                    <div>
                        <label class="text-xs text-zinc-400">Disponibilidad (unidades)</label>
                        <input name="disponibilidad" type="number" min="0" value="{{ old('disponibilidad', $recompensa->disponibilidad) }}" class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white focus:border-indigo-400 focus:ring-indigo-400" placeholder="Ej: 50 unidades">
                    </div>
